...
$timbrar = new SoapClient('http://pruebas.facturaselectronicascfdi.com/WsTimbrado33.asmx?WSDL');
//Creamos el objeto con el que se mandara a cancelar los folios
$cancelar = new SoapClient('http://pruebas.facturaselectronicascfdi.com/wsCancelar33.asmx?WSDL');
...
if(isset($_POST['SellarTimbrar33']))
{
	...
	$params = array('curCorreo' => $Correo, 'password' => $Pass,'xml' => $Xml, 'Proceso' => $Proceso,'CerFile' => $Cer, 'KeyFile' => $Key, 'PassCsd' =>$ClaveKey);
	try{
		$response = $timbrar->SellarTimbrar33($params);
			if (empty($response->SellarTimbrar33Result->errores))
			{
				$ResultText= '<br>El timbrado fue correcto <br> UUID: '.$response->SellarTimbrar33Result->folioUUID;
...
if(isset($_POST['Tibrar33']))
{
	...
	$Proceso = "410110";
	$ClaveKey = '12345678a';
	$params = array('curCorreo' => $Correo, 'password' => $Pass, 'xml' => $Xml,'Proceso' => $Proceso);
	//var_dump($params);
		try{
		$response = $timbrar->Timbrar33($params);
		//var_dump($response);
...
if(isset($_POST['Cancelar']))
{
	...
	try{
	$response = $cancelar->Cancelar33($params);
